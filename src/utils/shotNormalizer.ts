...
export const FIELD_ALIASES: Record<keyof Omit<Shot, "id" | "status" | "sourceRow">, string[]> = {
  sceneNumber: ["序", "場次", "場", "scene", "scene no", "scene number"],
  shotNumber: ["鏡號", "鏡次", "shot", "shot no", "鏡頭", "分鏡"],
  sceneTitle: ["場景", "場景名稱", "場次名稱", "scene title", "場名"],
  location: ["地點", "場景地", "location", "拍攝地點"],
  timeOfDay: ["日夜", "地點/時間", "時間", "time", "day/night", "時段"],
...
export function normalizeRow(row: Record<string, unknown>, mapping: Partial<Record<keyof Shot, string>>, index: number): Shot {
  ...
  const locationTime = get("timeOfDay");
  const locationTimeParts = locationTime.split(/[／/]/).map((part) => part.trim()).filter(Boolean);
  const inferredIntExt = locationTimeParts.find((part) => /^(內|外|INT|EXT)$/i.test(part)) || "";
  const inferredTime = locationTimeParts.find((part) => /^(日|夜|晨|昏|早|晚|DAY|NIGHT)$/i.test(part)) || locationTime;
  const rawLocation = get("location");
  return {
    id: `${sceneNumber}-${shotNumber}-${index}`,
    sceneNumber,
    shotNumber,
    sceneTitle: get("sceneTitle"),
    location: /^(內|外|INT|EXT)[／/]/i.test(rawLocation) ? get("sceneTitle") : rawLocation,
    timeOfDay: inferredTime,
    interiorExterior: intExt.includes("EXT") || intExt.includes("外") || inferredIntExt.includes("外") ? "EXT" : intExt.includes("INT") || intExt.includes("內") || inferredIntExt.includes("內") ? "INT" : "",
